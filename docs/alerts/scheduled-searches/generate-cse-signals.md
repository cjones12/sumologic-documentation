...
* 'threat_name'. This field should map to the alert name contained in the message
* 'description'. This field should contain a description of what the scheduled search provides. This is up to the user but a good rule of thumb is to keep the description to a minimum (less then 50 characters). 
    :::important
...
To view Signals that were created from a scheduled search, run a search using 'Vendor' is 'Sumo Logic' on the **Signals** page in the CSE UI.

Below is a screenshot of a Signal that was created from a scheduled search. Note that:

* The **Mapping** section at the bottom of the page shows that the Signal was the result of a Sumo Logic scheduled search.
